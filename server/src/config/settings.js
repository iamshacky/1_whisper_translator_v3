export const SELECT_LANGUAGE_MODE = true; // false = auto-detect (default), true = manual input language
export const DEFAULT_INPUT_LANG = 'en';    // Used when SELECT_LANGUAGE_MODE is true
export const DEFAULT_TARGET_LANG = 'es';   // Spanish as output

// === Speech Output Settings ===
export const SPEECH_MODE = 'synthesis'; // Options: 'tts', 'synthesis'

// Who should hear the audio? sender | receiver | both | none
export const PLAY_AUDIO_ON = 'both';
